...
function ProfilePage() {
  ...
  const [selectedimg, setseledtedimg] = useState(null)
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1' >
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            <input onChange={(e) => setseledtedimg(e.target.files[0])} type="file" name="" id="avatar" accept='.png, .jpg, .jpeg' hidden />
          ...
          <button type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 ext-white p-2 rounded-full text-lg cursor-pointer'>Save</button>
        </form>
