...
getUsers();
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5  overflow-y-sroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      ...
      <div className='flex flex-col gap-2'>
        {filteredUsers.map((user, index) => (
          <div
          onClick={()=>{setSelectedUser(user); setUnseenMessages(prev=>({...prev, [user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer text-sm overflow-hidden ${selectedUser?._id ===user._id && 'bg-[#282142]/50'}`}>
            <img src={user?.profilePic || assets.avatar_icon} alt="" className='h-9 w-9 rounded-full ' />
